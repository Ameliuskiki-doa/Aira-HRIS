...
**Isolation is the hard gate.** `tenant_id` on every table. Three allowlisted exemptions, not two: global `stat_*`, the pg-boss schema, and the two tables **at or above the tenant boundary** — `organizations` sits above it entirely, and `companies.id` *is* the `tenant_id`, so neither carries a `tenant_id` column. That exemption covers the column requirement only: both still need RLS enabled, forced, and a policy, keyed on `id` and on ownership respectively, RLS enabled *and* forced, tenant policy, and `tenant_id`-leading index — all in the same migration that creates the table. Policies wrap the claim as `(select public.tenant_id())`; without the subquery Postgres re-evaluates per row. The suite discovers tables from the catalog rather than a hand-maintained list, asserts a non-empty fixture result before asserting tenant purity, sweeps for tables missing RLS or a policy, and blocks merges. From this epic it also asserts **in-tenant** isolation per role tier.
...
- 1.4 gates every later migration: `public.tenant_id()`, the first policy pattern, and the catalog sweep must precede further tables; each new table extends the suite.
