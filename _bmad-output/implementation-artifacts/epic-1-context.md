...
**Styling.** Nocturne `styles.css` is vendored to `styles/nocturne.css` byte-identical and never edited — **token source only** (ramps, type, spacing, radius, elevation). Its nine component classes go unused. Components come from **shadcn on Radix**, copied into the repo, styled by Tailwind utilities resolving to Nocturne tokens through a **non-inline** `@theme`. Arbitrary values (`p-[13px]`, `bg-[#9184d9]`) are lint-forbidden. Dark is default (Nocturne `:root`); light is `[data-theme="light"]`, derived from the ramps. Eleven `--ui-*` semantic variables (`body, muted, faint, nav, hover, track, tint, active-bg, active-fg, accent-text, link-hover`) are **not** part of Nocturne and must be declared per theme in app CSS or screens render unstyled; `--ui-tint` keeps the dark accent hex in both themes. Set `color-scheme`; resolve the `localStorage` key `aira-theme` in a blocking inline script before first paint (do not port the artboard's JS variable injection — it flashes). Runtime switching requires the two-stage pattern: raw values on `:root` and the dark selector, mapped by the non-inline `@theme`; `@theme inline` breaks it.
