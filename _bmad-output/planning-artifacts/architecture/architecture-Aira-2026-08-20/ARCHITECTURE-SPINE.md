...
- **Rule:** Nocturne `styles.css` comes from the Claude Design project `dcaaa7ad-e795-4fad-8b3e-223f30a4ad1d` at `_ds/nocturne-ee56407c-8063-417c-bf1f-fe655f93985a/styles.css`, vendored to `styles/nocturne.css` byte-identical and re-fetched only deliberately. It is stored for provenance and checksum comparison, **not linked into the document** — its line-2 Google Fonts `@import` would otherwise fire on every visit, since PostCSS forwards remote `@import` URLs rather than inlining them. The app consumes its `:root` token block; **typography is Inter via `next/font/google`**, mapped onto `--font-heading` and `--font-body`. Geist is removed. It is the **token** source of truth — colours, ramps, type, spacing, radius, elevation. Its nine component classes go unused; components come from AD-36. The app-level `--ui-*` semantic layer is declared in application CSS beside it, per theme.
...
- **Rule:** Components come from **shadcn on Base UI**, copied into the repo, styled by Tailwind utilities that resolve to Nocturne tokens. Nocturne's component classes are **not** used alongside. **Arbitrary values are lint-forbidden** — `eslint-plugin-tailwindcss` `no-arbitrary-value`, scoped off for `components/ui/**`, which ships genuine arbitrary values by design. Dark mode is the **`.dark` class**, per shadcn's `@custom-variant dark (&:is(.dark *))`.
- **Theming:** `@theme inline` **plus `var()` indirection** to raw values declared on `:root` and `.dark`. Never literals inside `@theme inline` — Tailwind constant-folds them into the utility, leaving no variable to override, and the build still succeeds. *(Verified by experiment; an earlier version of this rule said the opposite and was wrong.)*
- **Adapter:** keep shadcn's semantic `@theme inline` block untouched and repoint only its raw `:root` / `.dark` layer at the Nocturne scale — components only ever name the semantic role. Note `--color-accent` collides: shadcn's is a muted surface, Nocturne's is the brand blurple.
